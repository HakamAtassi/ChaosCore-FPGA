...
`timescale 1 ns / 1 ps
	...
	module rocket_core_master_full_v1_0_M00_AXI #
	(
	...
	localparam [1:0] IDLE = 2'b00, // This state initiates AXI4Lite transaction 
		...
		INIT_COMPARE = 2'b11, // This state issues the status of comparison 
	        ...
	        WADDR = 2'b10, // This state initializes write address transaction 
	                       // once write address is done, the state machine  
	                       // changes state to WDATA 
	        WDATA = 2'b11, // This state initializes write data transactions 
	                       // once the written data are done, the state machine 
	                       // changes state to WADDR 
	        RADDR = 2'b10, // This state initializes read address transaction 
	                       // once read address is done, the state machine  
	                       // changes state to RDATA 
	        RDATA = 2'b11; // This state initializes read data transactions 
	                       // once the data are read, the state machine 
	            ...
	            begin                        
	              if (init_txn_pulse == 0 && mst_exec_state == INIT_WRITE)                        
	                begin                        
	                  ...
	                  axi_wvalid <= 1;                        
	                  axi_bready <= 0;                        
	                  if (C_M_AXI_BURST_LEN == 1) axi_wlast <= 1;                        
	                  else axi_wlast <= 0;                        
	                  state_write <= WADDR;                        
	                end                        
	              else                         
	...
	//--------------------------------
	            ...
	            begin                                                  
	              if (M_AXI_RVALID && M_AXI_RLAST && mst_exec_state != INIT_READ)                                                  
	                begin                                                  
	                  state_read <= IDLE;                                                  
	                  axi_arvalid <= 0;                                                  
	                  axi_rready <= 0;                                                  
	               ...
	               else if (M_AXI_RVALID && (read_index != C_M_AXI_BURST_LEN-1)) read_index <= read_index + 1;                                                  
	             end                                                  
	        endcase                                                  
	     end                                                                                  
	   end                                                  
	...
	// a parameterized threshold
	 ...
	 // committed.                                                                                              
	  ...
	  begin                                                                                                     
	    ...
	    //else if (M_AXI_BVALID && axi_bready && (write_burst_counter == {(C_NO_BURSTS_REQ-1){1}}) && axi_wlast)
	    else if (M_AXI_RVALID && axi_rready && (read_index == C_M_AXI_BURST_LEN-1) && (&read_burst_counter[C_NO_BURSTS_REQ-1:0]))
	      reads_done <= 1'b1;                                                                                   
	    else                                                                                                    
